...
def createNewDb():
    ...
    # Create table for players, without score_date cols (added to updateDB())
    #  due to lack of IF NOT EXISTS on ALTER TABLE commands.
    with con:
        cur = con.cursor()
...
def updateDb():
    ...
    with con:
        cur = con.cursor()
        try:
            cur.execute("ALTER TABLE players ADD COLUMN '" + DATESTAMP +
                        "_pc_gold' integer default 0")
        except SQLiteException:
            pass
        try:
            cur.execute("ALTER TABLE players ADD COLUMN '" + DATESTAMP +
                        "_pc_any' integer default 0")
        except SQLiteException:
            pass
        try:
            cur.execute("ALTER TABLE players ADD COLUMN '" + DATESTAMP +
                        "_n64_gold' integer default 0")
        except SQLiteException:
            pass
        try:
            cur.execute("ALTER TABLE players ADD COLUMN '" + DATESTAMP +
                        "_n64_any' integer default 0")
        except SQLiteException:
            pass
...
def get_il_leaderboard(game, category, level, medal_type='NONE'):
    # Takes in a GAME being run, a CATEGORY (e.g. X-Wing), a LEVEL (e.g. AaME)
...
def add_il_run_to_db(run_data):
    ...
    with con:
        cur = con.cursor()
        cur.execute("INSERT INTO il_runs_" + DATESTAMP + " (level, category, " +
                    " player, time, platform, medal, src_link)" +
                    " VALUES (?, ?, ?, ?, ?, ?, ?)", vals)
        ...
        for category in il_categories:
            ...
            if bad_il_combo:
                ...
                for medal in il_medals:
                    # Get a LIST of details for all runs fitting the level/cat
